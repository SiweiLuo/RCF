...
void test2D(int trig=2,int pt=3,int frame=0,int deltatheta=0,int deltaphi=0,double lthetaphi=0.) {
	...
	fsig->SetParameters(1,LBTHESIG+deltatheta*0.2,LBPHISIG+deltaphi*0.2,lthetaphi);
...
//	outputfile = new TFile(Form("~/polresults/20160707/MLEmethod/splot_2D_%d/splot_2D_%d_%d_%d_%d_%d_%d%d.root",time->GetDate(),trig,pt,frame,deltatheta,deltaphi,time->GetDate(),time->GetTime()),"recreate");
	outputfile = new TFile(Form("~/polresults/20160707/MLEmethod/splot_2D_%d/splot_2D_%d_%d_%d_%d_%d_%1.f_%d.root",time->GetDate(),trig,pt,frame,deltatheta,deltaphi,lthetaphi,time->GetDate()),"update");
