...
void mle(int ifile = 0,int itrig = 0, int xsect = 11, int ysect = 11,int rebin = 0){
...
//	histograms_copy = new TFile("templates20161107.root","read"); //40*40 bining
	...
	for(int ipt=0;ipt<6;ipt++){
		for(int iframe=0;iframe<2;iframe++){
			maxlikelihood(ifile,itrig,ipt,iframe,xsect,ysect,rebin);
		}
	}
}

void maxlikelihood(int ifile = 0,int itrig = 0,int ipt = 0,int iframe = 0,int xsect = 11,int ysect = 11,int rebin = 0){
	...
	for(int xnbin=xsect;xnbin<xsect+10;xnbin++){
		for(int ynbin=ysect;ynbin<ysect+10;ynbin++){
			...
			if(likelihood<=minlikelihood){
				...
				TRUTH->SetName(Form("template_%d_%d_%d_%d_theta_%d_phi_%d",ifile,itrig,ipt,iframe,xx-1,yy-1));
				TRUTH->Sumw2();
				if(rebin==1){
					TRUTH->RebinX(4);
					TRUTH->RebinY(4);
