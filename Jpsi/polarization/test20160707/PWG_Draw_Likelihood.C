...
Double_t* Lf2_theta_phi(Double_t* x, Double_t* par)
{
	lsig->SetParameter(0,1);
	lsig->SetParameter(1,x[0]);
	lsig->SetParameter(2,x[1]);
	lsig->SetParameter(3,0);// marker
...
void PWG_Draw_Likelihood(){
	...
	for(int frame=0;frame<1;frame++){
		for(int pt=3,trig=0;pt<5;pt++){
		
			if(pt==0) trig=0;
			else if(pt>=1 && pt<=2) trig=1;
			else if(pt>=3 && pt<=5) trig=3;
		
			cout<<" trig , pt , frame = "<<trig<<"   "<<pt<<"   "<<frame<<"   "<<endl;
		
//			correcteddata(sys,trig,pt,frame,1);
			read_data_eff(trig,pt,frame);
	cout<<" integral of data and effiicency = "<<datahist->Integral()<<"     "<<effhist->Integral()<<endl;
			theta_phi[pt][frame] = new TF2(Form("theta_phi_%d_%d",pt,frame),Lf2_theta_phi,-1,1,-1,1,0);
			theta_phi[pt][frame]->SetName(Form("func_%d_%d",pt,frame));
			//		Lf2_theta_phi->SetName(Form("func_%d_%d",pt,frame));
			theta_phi[pt][frame]->SetTitle("likelihood functional;#lambda_{#theta};#lambda_{#phi};");
			minimum = theta_phi[pt][frame]->GetMinimum();
			cout<<" minimum === "<<minimum<<endl;
			level[0] = minimum+0.5;
			c1->cd(frame*6+pt+1);
//			theta_phi[pt][frame]->SetContour(1,level);
//			theta_phi[pt][frame]->Draw("cont z ");
			theta_phi[pt][frame]->Draw("colz");
			c1->Update();
		}
	}

	//outputfile = new TFile(Form("~/polresults/20160707/functional/splot_3D_%d_2eid_inv30/PWG_Draw_L_3D_%d_%d_%d_%d_%d.root",time->GetDate(),trig,pt,frame,sys,time->GetDate()),"recreate");
	TF2* L_theta_phi = new TF2("L_theta_phi",Lf2_theta_phi,-1,1,-1,1,0);
	L_theta_phi->SetTitle("likelihood functional;#lambda_{#theta};#lambda_{#phi};");

	//	c1->cd();
	//	L_theta_phi->Draw("colz");
	c1->SaveAs("~/WWW/Proposal/figure/all_theta_phi.pdf");
	return;
...
void read_data_eff(int trig,int pt,int frame){

	TFile *file[12];
   	file[pt+frame*6]= new TFile(Form("/star/u/siwei/polresults/20160707/functional/splot_3D_20171201_2eid_inv30/splot_3D_%d_%d_%d_0_20171201.root",trig,pt,frame),"read");
	effhist = (TH2F*)file[pt+frame*6]->Get("effhist");
	effhist->SetName(Form("effhist_%d_%d_%d",trig,pt,frame));

	datahist = (TH2F*)file[pt+frame*6]->Get("datahist");
	datahist->SetName(Form("datahist_%d_%d_%d",trig,pt,frame));
...
void correcteddata(int file = 0,int trig,int pt,int frame,int rebin = 0){
	...
	for(;file<selectedfile;file++){
		...
		datahist = (TH2F*)rawdata2D[file][trig][pt][frame]->Clone(Form("datahist_%d_%d_%d_%d",file,trig,pt,frame));
		...
		effhist = (TH2F*)eff2D[file][trig][pt][frame][2]->Clone(Form("effhist_%d_%d_%d_%d",file,trig,pt,frame));
		/*
