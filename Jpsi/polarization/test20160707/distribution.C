...
TF2* fsigcompare=new TF2("fsigcompare","[0]*(1+[1]*y**2+[2]*(1-y**2)*cos(2*x)+[3]*cos(x)*y*(1-y**2)**0.5)",XMIN,XMAX,YMIN,YMAX);
...
TH2F *hunl, *hlik, *hsig, *hbkg, *hsub, *hsigcompare, *hunlcompare, *hsubcompare;
...
void pseudo(int n=0,int trig,int pt,int frame){
	...
	hunlcompare=new TH2F("hunlcompare","",NBIN,XMIN,XMAX,NBIN,YMIN,YMAX);
	hlik=new TH2F("hlik","",NBIN,XMIN,XMAX,NBIN,YMIN,YMAX);
	hsig=new TH2F("hsig","",NBIN,XMIN,XMAX,NBIN,YMIN,YMAX);
	hsigcompare=new TH2F("hsigcompare","",NBIN,XMIN,XMAX,NBIN,YMIN,YMAX);
	...
	for(int i=0;i<gRandom->Poisson(NSIGNAL);) {
		...
		if(gRandom->Uniform()<effhist->GetBinContent(effhist->FindBin(xsig,ysig))){
			hsig->Fill(xsig,ysig);
			hsigcompare->Fill(xsig1,ysig1);
			hunl->Fill(xsig,ysig);
			hunlcompare->Fill(xsig1,ysig1);
			hsub->Fill(xsig,ysig);
...
void distribution(int trig=2,int pt=3,int frame=0,int deltatheta=0,int deltaphi=0) {
	...
	fsigcompare->SetParameters(1,LBTHESIG+deltatheta*0.2,LBPHISIG+deltaphi*0.2,0.);
	...
	hlambda_err=new TH3F("hlambda_err","lambdas uncertainty;#lambda_{#theta};#lambda_{#phi};#lambda_{#theta#phi}",100,0,0.5,100,0,0.5,100,0,0.5);
	hlambda2_err=new TH3F("hlambda2_err","",100,0,0.5,100,0,0.5,100,0,0.5);

	TCanvas *c1=new TCanvas("tminuit","tminuit");
	c1->Divide(3,2);
	TCanvas *c2=new TCanvas("tf2","tf2");
	c2->Divide(3,2);

	outputfile = new TFile(Form("~/polresults/20160707/MLEmethod/splot_2D_%d/splot_2D_%d_%d_%d_%d_%d_%d%d.root",time->GetDate(),trig,pt,frame,deltatheta,deltaphi,time->GetDate(),time->GetTime()),"recreate");
	cout<<outputfile->GetName()<<endl;
	int trig=2,pt=3,frame=0;
	correcteddata(0,trig,pt,frame,1);

	pseudo(0,trig,pt,frame);
	c1->cd(1);
	hlambda->Draw();
	c1->cd(2);
	hlambda->ProjectionX("hlambda_px")->Draw();
	c1->cd(3);
	hlambda->ProjectionY("hlambda_py")->Draw();
	c1->cd(4);
	hlambda_err->Draw();
	c1->cd(5);
	hlambda_err->ProjectionX("hlambda_err_px")->Draw();
	c1->cd(6);
	hlambda_err->ProjectionY("hlambda_err_py")->Draw();
	c1->SaveAs(Form("~/polresults/20160707/MLEmethod/splot/c1_%d.pdf",time->GetDate()));

	c2->cd(1);
	hlambda2->Draw();
	c2->cd(2);
	hlambda2->ProjectionX("hlambda2_px")->Draw();
	c2->cd(3);
	hlambda2->ProjectionY("hlambda2_py")->Draw();
	c2->cd(4);
	hlambda2_err->Draw();
	c2->cd(5);
	hlambda2_err->ProjectionX("hlambda2_err_px")->Draw();
	c2->cd(6);
	hlambda2_err->ProjectionY("hlambda2_err_py")->Draw();
...
void minuitfit(){
	...
	static Double_t vstart[4] = {0., 0. , 0. , 0.01};
	...
	hlambda->Fill(getlambdatheta,getlambdaphi,0);
	hlambda_err->Fill(getlambdathetaerr,getlambdaphierr,0);
}
...
void tf2fit(){
	...
	TF2* flikelihood=new TF2("flikelihood",likelihoodfcn,-1,1,-1,1,3);
	flikelihood->Write("",TObject::kOverwrite);
	...
	if(contlevel->First()==NULL) {
		...
		hlambda2->Fill(lbthesig2,lbphisig2,0);
		...
		if(contlevel2->First()==NULL) {
			...
			hlambda2_err->Fill(lbthesig2_err,lbphisig2_err,0);
		}
