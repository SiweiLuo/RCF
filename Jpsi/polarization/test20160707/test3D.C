...
void test3D(int trig=2,int pt=3,int frame=0,int deltatheta=0,int deltaphi=0,int deltathetaphi) {
	...
	fsig->SetParameters(1,LBTHESIG+deltatheta*0.2,LBPHISIG+deltaphi*0.2,-0.1+deltathetaphi*0.2);
	fbkg->SetParameters(1,LBTHEBKG,LBPHIBKG);
	hlambda=new TH3F("hlambda1","lambdas;#lambda_{#theta};#lambda_{#phi};#lambda_{#theta#phi}",100,-1,1,100,-1,1,100,-1,1);
	...
	outputfile = new TFile(Form("~/polresults/20160707/MLEmethod/splot_3D_%d/splot_3D_%d_%d_%d_%d_%d_%d_%d.root",time->GetDate(),trig,pt,frame,deltatheta,deltaphi,deltathetaphi,time->GetDate()),"recreate");
