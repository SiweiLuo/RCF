...
void jpsiEff(const char* fileInDir="./out_Jpsi_200", const char* OutFile="jpsiEff_200", const int mCentCut1=0, const int mCentCut2=9){
	...
	for (int i = 0; i < nevents; i++) {
		...
		for(int j=0;j<mElectronEvent->nReal();j++){
			...
			for(int k=j+1; k<mElectronEvent->nReal();k++) {
				...
				if(mElectron->id>=0 && mElectron2->id>=0 &&mElectron->pt>0 && mElectron2->pt>0 ){ jpsiRc = ePosRc + eNegRc;
	...
	hmcPtvsrcPt->Write();
	hmcPtvsrcPt_Cut->Write();
	hRcJpsiPt->Write();
	hRcJpsiPt_Or->Write();
	hRcJpsiY->Write();
	hRcJpsiPtY_B->Write();
	hRcJpsiPtY_BOr->Write();
	hRcJpsiPtY->Write();
	hRcJpsiPtY_Or->Write();
	hRcJpsiPhi->Write();
	hRcJpsiPtM->Write();
	...
	hMCdeltaEtavsdeltaPhi->Write();	
	hRCdeltaEtavsdeltaPhi->Write();	
	hdeltaEtavsMCPt->Write();
	hdeltaPhivsMCPt->Write();
	hdeltaEtavsRCPt->Write();
	hdeltaPhivsRCPt->Write();
	hdeltaRvsMCPt->Write();
	hdeltaRvsRCPt->Write();

	hHt2Adc0vsPt->Write(); 
	hHt2Adc0vsrcPt->Write();
